(ns rumaterial.component.message
...
(rum/defc message-content < rum/static [className text icon]
  (comp/snackbar-content
    {:aria-describedby "message-snackbar"
     :className        className
     :message          (html [:span.rumaterial-message#message-snackbar icon text])
...
(rum/defc info-message < rum/reactive [text opened?]
  (comp/snackbar
    {:autoHideDuration 6000
     ...
     :open             opened?
     :onClose          #(state/update-value [:open] false message-cursor)}
    (message-content "rumaterial-label-green" text (icon/check-circle {:className "rumaterial-message-icon"}))))
...
(rum/defc error-message < rum/reactive [text opened?]
  (comp/snackbar
    {:autoHideDuration 300000
     ...
     :open             opened?
     :onClose          #(state/update-value [:open] false message-cursor)}
    (message-content "rumaterial-label-red" text (icon/error {:className "rumaterial-message-icon"}))))
